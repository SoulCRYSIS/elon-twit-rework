"""
HistGradientBoosting YES model (sklearn): EV-weighted + PnL threshold tuning.

Train:
  python scripts/train_ml_model.py --classifier hgb --ev-weighted --tune-on pnl
Writes: data/ml_artifacts/hgb_ev.joblib, meta_hgb_ev.json

Backtest / bot: --approach hgb_ev
"""
    ...
    **kwargs,
    ...
    if b.get("missing"):
        return Signal(
            ...
            edge=0.0,
            metadata={"error": "Run: python scripts/train_ml_model.py --classifier hgb --ev-weighted --tune-on pnl"},
        )
