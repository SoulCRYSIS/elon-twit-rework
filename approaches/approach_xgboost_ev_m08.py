"""
XGBoost YES model: EV-weighted + PnL threshold tuning, artifact tag ``xgb_ev_pnl_m08``.

Trained via:
  python scripts/train_ml_model.py --classifier xgboost --artifact-tag xgb_ev_pnl_m08 \\
    --ev-weighted --tune-on pnl --max-buy-rate 0.08

Backtest: bind ``make_get_signal`` to these paths, or ``--approach xgboost_ev_m08`` in bot.
"""
    ...
    **kwargs,
    ...
    if b.get("missing"):
        return Signal(
            ...
            edge=0.0,
            metadata={"error": "Train xgb_ev_pnl_m08 artifact or run scripts/run_ml_experiments.py"},
        )
