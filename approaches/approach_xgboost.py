"""
XGBoost YES model: load offline-trained artifacts (temporal split, 7d-only, pruned features).

Train with: python scripts/train_ml_model.py
If artifacts are missing, falls back to a conservative constant-probability signal (no look-ahead).
"""
...
def get_signal(
    ...
    position: dict | None = None,  # ignored for entry; set by engine for paired buy/sell context
    **kwargs,
) -> Signal:
    ...
    if start_time is not None:
        ...
        # Biased fallback: unknown start — assume 7d window ending at `end`
        start = end - 7 * 24 * 3600

    b = _load_bundle()
    if b.get("missing"):
        # No artifact: do not fabricate strong edges (would be misleading in backtest)
        return Signal(
            buy=False,
            sell=False,
            confidence=0.0,
